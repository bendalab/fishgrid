...
void usage( void )
{
  cout << "FishGridStepper " << FISHGRIDVERSION << endl;
  cout << "Copyright (C) 2011 Jan Benda & Joerg Henninger\n";
  cout << "\n";
  cout << "Usage:\n";
  cout << "\n";
  cout << "FishGridStepper -f SAMPLINGRATE -g GAIN -v MAXVOLT -b BUFFERTIME -d DATATIME -n -m -3\n";
  cout << "\n";
  ...
  cout << "-d DATATIME         size of the data segements in milliseconds on which analysis operates on\n";
...
int main( int argc, char **argv )
{
  ...
  int longindex = 0;
  char c;
  while ( (c = getopt_long( argc, argv, "f:g:v:b:d:p:mn3", longoptions, &longindex )) >= 0 ) {
    switch ( c ) {
  ...
  Stepper fishgrid( samplerate, maxvolts, gain,
