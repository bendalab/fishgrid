...
DAQFlexCore::DAQFlexCore( const string &device  )
  : DeviceHandle( NULL ),
    ErrorString( "" ),
    ErrorState( Success )
{
...
int DAQFlexCore::open( const string &devicestr, int mccdevicenum, const string &firmwarepath )
{
  clearError();
...
void DAQFlexCore::setErrorStr( const string &strg )
{
  ErrorString = strg;
}


void DAQFlexCore::addErrorStr( const string &strg )
{
  if ( strg.empty() )
    return;
  if ( !ErrorString.empty() ) {
...
void DAQFlexCore::setErrorStr( int errnum )
{
  char buf[1000];
...
void DAQFlexCore::addErrorStr( int errnum )
{
  if ( !ErrorString.empty() )
    ErrorString += ", ";
...
string DAQFlexCore::getErrorStr( int ern )
{
  string es = "";
  switch ( ern ) {
  case NotOpen :
...
{
  return ( ErrorState == Success && ( ErrorString.empty() ) );
}
...
void DAQFlexCore::lock( void ) const
{
  Mutex.lock();
}


void DAQFlexCore::unlock( void ) const
{
  Mutex.unlock();
}


QMutex *DAQFlexCore::mutex( void ) const
{
  return &Mutex;
}
