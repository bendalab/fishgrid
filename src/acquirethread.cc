...
AcquireThread::AcquireThread( FishGridWidget *fgw )
  : DataThread( "Acquisition", fgw ),
    Handle( 0 )
{
...
int AcquireThread::initialize( void )
{
  int32 error = 0;
  char errstr[2048];
  ...
  printlog( "AcquireThread interval=" + Str( 1000.0*interval() ) + "ms" );
...
void AcquireThread::finish( void )
{
  DAQmxBaseStopTask( Handle );
  DAQmxBaseClearTask( Handle );
}
